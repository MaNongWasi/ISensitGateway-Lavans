...
#		print "cursor ", cursor.rowcount
    ...
    def insert_acc_beacon_data(self, beacon_id, beacon_accx, beacon_accy, beacon_accz, beacon_rssi, bad_lift, total_lift, state, currenttime, small_count, middle_count, large_count):
        try:
            with self.connection.cursor() as cursor:
                # Create a new record
                sql = "INSERT INTO " + self.table + " VALUES (NULL, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP, %s, %s, %s, %s, %s, %s)"
                cursor.execute(sql, (beacon_id, beacon_accx, beacon_accy, beacon_accz, beacon_rssi, bad_lift, total_lift, state, small_count, middle_count, large_count))
...
#       print delete_stmt, "  ", row_count
    ...
    def read_cal_val(self, beacon_id):
        try:
            with self.connection.cursor() as cursor:
                # Create a new record
                cursor.execute("SELECT * from users where beacon_id = %s", (beacon_id))
            ...
            if cursor.rowcount > 0:
                return cursor.fetchone()["cal_val"]
            else:
                return None
